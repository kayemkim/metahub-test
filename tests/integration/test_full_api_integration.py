...
class TestFullAPIIntegration:
    ...
    def test_content_versioning_workflow(self, test_client):
        ...
        if bootstrap_response.status_code != 200:
            pytest.skip(f"Bootstrap failed: {bootstrap_response.status_code} - {bootstrap_response.json()}")

        # Terms 조회
        terms_response = test_client.get("/api/v1/taxonomy/DATA_DOMAIN/terms")
        if terms_response.status_code != 200:
            pytest.skip(f"Cannot get terms: {terms_response.status_code} - {terms_response.json()}")
            
        terms = terms_response.json()
        if not terms or len(terms) == 0:
            pytest.skip(f"No terms available: got {terms}")
